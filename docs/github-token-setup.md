...
notes: How to create a secure, least-privilege GitHub token for Herald (fine-grained PAT preferred), where to store it, and the security model. Linked from the README.
---

# GitHub token setup

Herald reads your organization's GitHub activity (commits, pull requests,
...
> **The token never leaves your machine.** Herald is code you run yourself, not a
> hosted service. The token sits in a local `.env` (gitignored) and is sent only
> to `github.com`. Nothing is transmitted to the Herald authors, to Anthropic, or
...
6. **Repository permissions** — set these to **Read-only**, leave everything else
   *No access*:
   ...
   | Permission | Access | Why Herald needs it |
   |---|---|---|
...
Herald reads `GITHUB_TOKEN` (or `GH_TOKEN`) from the environment.
...
- **Never committed** — `.env` and `herald.config.json` are gitignored.
...
When Herald runs as a hosted service for teams (Phase 6), it will **not** ask
