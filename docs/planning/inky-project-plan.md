...
- **Phases 0–4 complete + Phase 5 MVP** (GitHub-milestones `reconcile()`), all committed on `main`. **99 tests passing, typecheck clean.**
...
- **LIVE & HOSTED (2026-06-03):** public OSS at `github.com/Doppel-Labs/inky` (MIT, history scrubbed); deployed as a **Render Background Worker** (`render.yaml`) posting a **daily (9am PT weekdays) + weekly (8am PT Mon)** standup on its own, plus on-demand **`/standup`** (the bot is live — `Inky#0459`). Token = least-privilege fine-grained PAT. Config covers all org repos with `staleDays: "auto"`. The whole MVP vision is realized and running.
