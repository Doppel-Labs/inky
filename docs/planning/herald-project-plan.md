...
- **Phases 0–4 complete** (MVP done), all committed on `main`. **78 tests passing, typecheck clean.**
...
- **Working commands:** `herald collect`; `herald standup --dry-run [--days N|--hours N] [--provider p] [--model m] [--format prose|bullets] [--stats|--no-stats] [--stats-per-person] [--mechanical]`; `herald serve [--once] [--dry-run]`; `herald register-commands`. Dev run: `GITHUB_TOKEN=$(gh auth token) pnpm --silent dev <cmd> …`. Outputs saved to gitignored `.herald-output/`.
...
- **Next:** MVP (Phases 0–4) is feature-complete; remaining is a **live run** (user provides `DISCORD_WEBHOOK_URL` and/or `DISCORD_BOT_TOKEN`+`applicationId`). Then **Phase 5** (`reconcile()` — status vs roadmap, the paid hook), or quick adds: PR size distribution stat, a review/cleanup pass.
